...
mispriced order cannot invent a fake margin. Items are ranked by ISK per m3 (the headline metric
for hauling). Tax, broker fee and freight are editable on the page; the bake stores raw prices and
nets them client side, so the operator tunes them without a rebuild.
...
`.github/workflows/run.yml` runs hourly, persists `ids_cache.json` + `vol_cache.json` back to this
repo, and publishes the page to `CrownOak/wdeve` at `/arbitrage/` via the `WDEVE_DEPLOY_KEY` deploy
key. Requires repo secrets: `WDEVE_DEPLOY_KEY` (required), `EVE_PAGE_PASSWORD` (optional, unused for
the public page).
