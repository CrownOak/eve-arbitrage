...
(function(){{
  ...
  function optimize(){{
    ...
    D.forEach(function(r){{
      if(!(r.m3>0)) return;
      var sxp=r.sxp||[], bxp=r.bxp||[], dvx=r.dvx||[];
      var buy=sxp[oi]; if(buy==null) return;
      var sell, fee;
      if(model==="flip"){{ sell=bxp[di]; fee=tax; }} else {{ sell=sxp[di]; fee=tax+broker; }}  // flip into buy orders / relist at dest
      if(sell==null) return;
      var net=sell*(1-fee), pu=net-buy;
      if(pu<=0) return;
      var cap=Math.floor(days*Math.min(dvx[oi]||0, dvx[di]||0));   // capped at a day of real traded volume
      if(cap<1) return;
  ...
  function renderLoad(origin,dest,cargo,left,load,totCost,totProfit){{
    ...
    h+="<div class='mfoot'><button id='loCopy' class='link'>copy origin multibuy</button><span>buy at "+origin+", haul to "+dest+", "+(st.loModel==="flip"?"dump into buy orders":"relist under the market")+". Prices use the robust 5% and quantities are capped at a day of what each hub actually trades, so the load is realistic, but depth is not modeled and a big haul still moves the market. Estimate, not a promise.</span></div>";
    out.innerHTML=h;
